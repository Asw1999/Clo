...
<template>
	<div class="flex w-full flex-col gap-3 sm:flex-row sm:items-center">
		<div class="grid min-w-0 flex-1 grid-cols-3 gap-2 sm:flex sm:flex-wrap sm:items-center">
			<div class="relative min-w-0">
				<button type="button" class="inline-flex w-full items-center justify-between gap-1 rounded-xl border border-[#e0e3e7] bg-[#f8fafd] px-2 py-2 text-xs font-medium text-[#3c4043] transition hover:border-[#c7d2e0] hover:bg-white dark:border-slate-700 dark:bg-slate-900/70 dark:text-slate-200 dark:hover:bg-slate-800 sm:w-auto sm:justify-start sm:gap-2 sm:rounded-2xl sm:px-3.5 sm:py-2.5 sm:text-sm" @click.stop="emit('toggle-filter-menu', 'type')">
					<span class="truncate">{{ getFilterLabel('type', selectedTypeFilter) }}</span>
					<IconX v-if="isFilterActive('type')" :size="14" :stroke="2" class="shrink-0 text-[#5f6368] transition hover:text-[#1a73e8] dark:text-slate-400 dark:hover:text-sky-300 sm:size-4" @click.stop="emit('clear-filter', 'type')" />
					<IconChevronDown v-else :size="14" :stroke="2" class="shrink-0 text-[#5f6368] dark:text-slate-400 sm:size-4" />
				</button>
			...
			<div class="relative min-w-0">
				<button type="button" class="inline-flex w-full items-center justify-between gap-1 rounded-xl border border-[#e0e3e7] bg-[#f8fafd] px-2 py-2 text-xs font-medium text-[#3c4043] transition hover:border-[#c7d2e0] hover:bg-white dark:border-slate-700 dark:bg-slate-900/70 dark:text-slate-200 dark:hover:bg-slate-800 sm:w-auto sm:justify-start sm:gap-2 sm:rounded-2xl sm:px-3.5 sm:py-2.5 sm:text-sm" @click.stop="emit('toggle-filter-menu', 'owner')">
					...
					<IconX v-if="isFilterActive('owner')" :size="14" :stroke="2" class="shrink-0 text-[#5f6368] transition hover:text-[#1a73e8] dark:text-slate-400 dark:hover:text-sky-300 sm:size-4" @click.stop="emit('clear-filter', 'owner')" />
					<IconChevronDown v-else :size="14" :stroke="2" class="shrink-0 text-[#5f6368] dark:text-slate-400 sm:size-4" />
				</button>
				<div v-if="activeFilterMenu === 'owner'" class="absolute left-1/2 top-full z-30 mt-2 min-w-[260px] -translate-x-1/2 sm:left-0 sm:translate-x-0 overflow-hidden rounded-2xl border border-[#e0e3e7] bg-white p-2 shadow-[0_16px_40px_rgba(32,33,36,0.16)] dark:border-slate-700 dark:bg-slate-800">
			...
			<div class="relative min-w-0">
				<button type="button" class="inline-flex w-full items-center justify-between gap-1 rounded-xl border border-[#e0e3e7] bg-[#f8fafd] px-2 py-2 text-xs font-medium text-[#3c4043] transition hover:border-[#c7d2e0] hover:bg-white dark:border-slate-700 dark:bg-slate-900/70 dark:text-slate-200 dark:hover:bg-slate-800 sm:w-auto sm:justify-start sm:gap-2 sm:rounded-2xl sm:px-3.5 sm:py-2.5 sm:text-sm" @click.stop="emit('toggle-filter-menu', 'updated')">
					...
					<IconX v-if="isFilterActive('updated')" :size="14" :stroke="2" class="shrink-0 text-[#5f6368] transition hover:text-[#1a73e8] dark:text-slate-400 dark:hover:text-sky-300 sm:size-4" @click.stop="emit('clear-filter', 'updated')" />
					<IconChevronDown v-else :size="14" :stroke="2" class="shrink-0 text-[#5f6368] dark:text-slate-400 sm:size-4" />
				</button>
